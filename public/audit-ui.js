(()=>{
 ...
 // One-use boot replay: app.js currently asks for today's dashboard twice during
 // initialisation. Re-use the first successful response once, then return to normal.
 ...
 document.addEventListener('click',e=>{const b=e.target.closest('[data-action],[data-session-menu],[data-past-action]');if(!b)return;if(b.dataset.auditClickBusy){e.preventDefault();e.stopImmediatePropagation();return}b.dataset.auditClickBusy='1';b.disabled=true;setTimeout(()=>{if(b.isConnected){b.disabled=false;delete b.dataset.auditClickBusy}},8000)},true);
 ...
 function cleanActionWords(){document.querySelectorAll('#sessionActionList button').forEach(b=>{if(b.textContent.includes('رجوع لمجدولة'))b.childNodes[0].nodeValue='التراجع عن تسجيل الحصة';if(b.textContent.includes('إرجاع الحصة'))b.childNodes[0].nodeValue='إعادة الحصة للجدول'});const t=document.getElementById('toastText');if(t?.textContent==='رجعت لمجدولة')t.textContent='تم التراجع عن تسجيل الحصة'}
 ...
 function renderStudentAccount(){const body=document.getElementById('studentProfileBody'),data=accountCache.get(lastAccountId);if(!body||!data?.student||!body.querySelector('.student-summary-grid'))return;const s=data.student;if(body.dataset.auditAccount===String(lastAccountId)&&body.dataset.auditStamp===String((data.monthly_dues||[]).length))return;body.dataset.auditAccount=String(lastAccountId);body.dataset.auditStamp=String((data.monthly_dues||[]).length);
  ...
  const timeline=body.querySelector('.profile-section .timeline-mini');
  const timelineSections=[...body.querySelectorAll('.profile-section')].filter(x=>x.querySelector('h4')?.textContent==='آخر الحركة');const target=timelineSections[0]?.querySelector('.timeline-mini')||timeline;
