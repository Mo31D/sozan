...
const UI_SCRIPTS='<script type="module" src="/schedule-view.js"></script><script type="module" src="/reports-view.js"></script>';
...
export default{
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/api/health'){
      return json({ok:true,app:env.APP_NAME||'Sozan Tutor OS',version:'4.7',mutation_dedupe:true,receipt_rebalance:true,schedule_views:true,reports:true});
    }

    if(url.pathname==='/api/v4/reports'&&request.method==='GET'){
      const auth=await checkAuth(request,env,ctx);
      if(!auth.ok)return auth.response;
      return buildReport(url,env);
    }
    ...
    if((url.pathname==='/'||url.pathname==='/index.html')&&request.method==='GET'){
      ...
      if(!html.includes('/schedule-view.js')){
...
async function checkAuth(request,env,ctx){
  const probeUrl=new URL('/api/v3/settings',request.url);
...
async function buildReport(url,env){
  ...
  if(!month)return json({error:'اختاري شهر صحيح'},400);

  const [sessionSummary,directCash,receiptCash,otherIncome,expenseSummary,outstanding,sessionsByType,expensesByCategory,otherIncomeByCategory]=await Promise.all([
    ...
    env.DB.prepare(`SELECT r.session_type key,COALESCE(SUM(o.earned_pence),0) amount_pence,COUNT(*) count FROM session_occurrences_v3 o JOIN recurring_sessions_v3 r ON r.id=o.recurring_session_id WHERE o.status='completed' AND substr(COALESCE(o.rescheduled_to_date,o.session_date),1,7)=?1 GROUP BY r.session_type ORDER BY amount_pence DESC`).bind(month).all(),
  ...
  const studentCash=Number(directCash?.value||0)+Number(receiptCash?.value||0),other=Number(otherIncome?.value||0),expenses=Number(expenseSummary?.total||0);
  return json({month,summary:{session_earned_pence:Number(sessionSummary?.earned_pence||0),completed_sessions:Number(sessionSummary?.completed_sessions||0),student_cash_pence:studentCash,other_income_pence:other,expenses_pence:expenses,business_expenses_pence:Number(expenseSummary?.business||0),personal_expenses_pence:Number(expenseSummary?.personal||0),cash_net_pence:studentCash+other-expenses,month_outstanding_pence:Number(outstanding?.value||0)},sessions_by_type:sessionsByType.results||[],expenses_by_category:expensesByCategory.results||[],other_income_by_category:otherIncomeByCategory.results||[]});
}
